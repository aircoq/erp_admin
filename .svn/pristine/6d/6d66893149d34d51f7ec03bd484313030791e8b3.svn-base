...
class AmazonHeelSaleSync extends AbsTasker
{
    public function getName()
    {
        return "Amazon跟卖状态同步-停止使用";
    }
                        ...
                        if(isset($val['id']) && $val['id']){
                            ...
                            if($id){
                                ...
                                $data = [
                                    ...
                                    'status' => 1,
                                    'create_time' => time(),
                                    'amazon_listing_id' => $id
                                ];
                                $actionLogModel->insertGetId($data);


                                $object->update(['status' => 1, 'last_request_time' => $time, 'listing_id' => $id], ['id' => $val['id']]);
