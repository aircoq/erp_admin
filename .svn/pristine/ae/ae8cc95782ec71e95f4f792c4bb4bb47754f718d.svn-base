...
class JumiaAccountService
{
    ...
    public function getList(array $req)
    {
        ...
        $field = 'am.id,am.account_name,am.code,am.is_invalid,am.jumia_enabled,am.create_time,am.update_time,am.is_authorization,am.sync_delivery,am.base_account_id,am.download_order,am.creator_id,am.updater_id,am.download_listing,s.site_status,c.seller_id,c.customer_id,a.register_time,a.fulfill_time';
        //有数据就取出
    ...
    public function changeStatus($data)
    {
        ...
        try {
            ...
            if (isset($data['status'])) {
                ...
                if($data['status'] == 0){
                    $updata['download_order'] = 0;
                    $updata['download_listing'] = 0;
                    $updata['sync_delivery'] = 0;
                    $res['isChange'] = true;
                }
            }
            ...
            $this->jumiaAccountModel->allowField(true)->save($updata, ['id' => $data['id']]);

            self::addJumiaLog($operator, ChannelAccountLog::UPDATE, $new_data, $old_data);
