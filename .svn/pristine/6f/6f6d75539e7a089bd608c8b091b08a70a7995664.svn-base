...
class Excel
{
    ...
    public static function exportExcel2007($header, $lists, $file, $isQueue = 0)
    {
        ...
        if(array_column($lists,'id') && !in_array('id', $fieldKeys)){
            $fieldKeys[] = 'id';
        }
        $mainNeedMergerKey = isset($mainNeedMergerKey)?$mainNeedMergerKey:'id';
        $countCol = count($title);//写入标题列数
        $writer->writeSheetHeader('Sheet1', $title, ['font-style'=>'bold', 'widths'=>$colWidths]);
...
//                        if (isset($list[$key-1])){
//                            $lastRow[$keyName] = $list[$key-1][$keyName];//上一行
//                        }
                        if (count($row) == $countCol){
                            break;//写入列达到标题最大列，多余数据不取
                        }
                    }
//                    if (isset($lastRow) && !array_diff($row,$lastRow)){
//                        continue;
//                    }
                    $writer->writeSheetRow('Sheet1', $row, ['valign'=>'center']);
                }
                $endRow = $writer->countSheetRows() - 1;//结束行数
                //合并行数
                foreach ($needMerge as $key){
                    $writer->markMergedCell('Sheet1',$startRow, $key, $endRow,$key);
                }
            }
        }else{
            foreach ($lists as $list){
