...
class AmazonPublishService
{
    ...
    /**
     * @var array SearchTerms里面禁用的商标用语；
     */
    public $trademarkeds = ['apple', 'samsung', 'dell', 'iphone', 'macbook', 'furminator'];
    ...
    public function checkSearchTerms($field, $lang = 'zh')
    {
        ...
        foreach ($this->trademarkeds as $val) {
            $val = ' '. $val. ' ';
    ...
    public function saveProductDetailData($data): string
    {
        ...
        if ($product['id'] == 0) {
            ...
            $task_id = AmazonPublishTask::where(['goods_id' => $product['goods_id'], 'account_id' => $product['account_id'], 'status' => ['IN', [0, 1, 3]]])->value('id');
