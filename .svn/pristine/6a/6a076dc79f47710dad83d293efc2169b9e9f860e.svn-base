...
class AliAuthCategory extends Base{
    ...
    /**
     * @title 嵌入式营销模块列表
     * @url /aliexpress-marketing-module-list
     * @method get
     * @return json
     */
    public function marketingModuleList()
    {
        $param = $this->request->instance()->param();
        $page = $this->request->param('page',1);
    ...
    /**
     * @title 添加嵌入式营销模块
     * @url /aliexpress-add-marketing-module
     * @method post
     * @return json
     */
    public function addMarketingModule(Request $request)
    {
        try{

            $data = $request->post();
            if(!$data) {
                return json(['message'=>'提交的数据不能为空'],500);
            }


            $result = $this->service->create_embed_product_template($data);
            if($result['status']) {
    ...
    /**
     * @title 删除嵌入式营销模块
     * @url /aliexpress-del-marketing-module
     * @method post
     * @return json
     */
    public function delMarketingModule(Request $request)
    {
        try{
    ...
    /**
     * @title 编辑嵌入式营销模块
     * @url /aliexpress-edit-marketing-module
     * @method post
     * @return json
     */
    public function editMarketingModule()
    {
        try{
    ...
    /**
     * @title SPU和账号搜索已刊登商品
     * @url /aliexpress-search-publish-info
     * @method get
     * @return json
     */
    public function searchPublishInfo(Request $request)
    {
        try{

            $data = $request->param();
            if(!$data) {
                return json(['message'=>'请输入该账号已刊登过的SPU'],500);
            }


            $result = $this->service->searchPublishInfo($data);
            if($result['status']) {
