 ...
 * Time: 11:43
 */
class ZoodmallAccountService
{
    protected $zoodmallAccountModel;
    protected $error = '';
    ...
    {
        if (is_null($this->zoodmallAccountModel)) {
            $this->zoodmallAccountModel = new ZoodmallAccountModel();
        }
    }
    ...
    public function getList(array $req)
    {
        ...
        $count = $this->zoodmallAccountModel
            ->alias('am')
            ->where($where)
        ...
        $field = 'am.id,am.account_name,am.code,am.is_invalid status,am.create_time,am.update_time,am.is_authorization,am.sync_delivery,am.base_account_id,am.download_order,am.creator_id,am.updater_id,am.download_listing,s.site_status,c.seller_id,c.customer_id,a.register_time,a.fulfill_time';
        //有数据就取出
        $list = $this->zoodmallAccountModel
            ->alias('am')
            ->field($field)
            ->join('__ACCOUNT__ a', 'a.id=am.base_account_id', 'LEFT')
            ->join('__CHANNEL_USER_ACCOUNT_MAP__ c', 'c.account_id=am.id AND c.channel_id=a.channel_id', 'LEFT')
    ...
    public function accountList($params = [], $page = 1, $pageSize = 10)
    {
        ...
        $field = 'id,account_name,code,is_invalid status,create_time,update_time,is_authorization,sync_delivery,download_order,download_listing';
        ...
        $count = $this->zoodmallAccountModel->field($field)->where($where)->count();
        $accountList = $this->zoodmallAccountModel->field($field)->where($where)->order($sort)->page($page, $pageSize)->select();
    ...
    public function add($data, $uid = 0)
    {
        try {
            ...
            if ($id == 0) {
                //检查产品是否已存在
                if ($this->zoodmallAccountModel->check(['account_name' => $data['account_name']])) {
                    $this->error = $data['account_name'].'账号已经存在无法重复添加';
                    return false;
                }
                if ($this->zoodmallAccountModel->check(['code' => $data['code']])) {
                    $this->error =  $data['code'].'zoodmall简称已经存在无法重复添加';
                    return false;
                }
                ...
                //必须要去账号基础资料里备案
                \app\index\service\BasicAccountService::isHasCode(ChannelAccountConst::channel_Zoodmall,$data['code']);

            } else {
                $is_ok = $this->zoodmallAccountModel->field('id')->where(['code' => $data['code']])->where('id', '<>', $id)->find();
                if ($is_ok) {
                    $this->error = 'zoodmall简称已经存在无法修改';
                    return false;
                }

                $old_data = $this->zoodmallAccountModel::get($id);
            ...
            $save_id = $this->zoodmallAccountModel->add($save_data);
            ...
            $save_id and self::addZoodmallLog(
                $operator, 
    ...
    {
        $field = 'id,account_name,code,is_invalid status,base_account_id,create_time,update_time,is_authorization,sync_delivery,download_order,download_listing';
        if ($id == 0) {
            return $this->zoodmallAccountModel->field($field)->order('id desc')->find();
        }
        return $this->zoodmallAccountModel->where('id', $id)->field($field)->find();
    }
    ...
    {
        return $this->zoodmallAccountModel->where('id', $id)->field('id,code,account_name,client_id,client_secret')->find();
    }
    ...
    public function changeStatus($data)
    {
        ...
        try {
            ...
            $res = $this->zoodmallAccountModel->allowField(true)->save($updata, ['id' => $data['id']]);
    ...
    public function updateToken($data, $uid = 0)
    {
        ...
        try {
            ...
            $res = $this->zoodmallAccountModel->add($save_data);
            (new ZoodmallOrderApi($account))->refresh_access_token();
     ...
     */
    public function updateAccessToken($id,$accessToken)
    {
        $cache = Cache::store('ZoodmallAccount');
        $where['id'] = $id;
        $save_data['access_token'] = $accessToken;
        $save_data['is_authorization'] = 1;
        $save_data['update_time'] = time();
        $this->zoodmallAccountModel->save($save_data,$where);
        //更新缓存
        foreach ($save_data as $key => $val) {
            $cache->updateTableRecord($id, $key, $val);
        }
     ...
     * @author lingjiawen
     */
    public static function addZoodmallLog(array $base_info = [], int $type = 0, array $new_data = [], $old_data = []): void
    {
        $insert_data = [];
        $remark = [];
        ...
        $insert_data and ChannelAccountLog::addLog([
            'channel_id' => ChannelAccountConst::channel_Zoodmall,
     ...
     * 获取日志
     */
    public function getZoodmallLog(array $req = []): array
    {
        $page = isset($req['page']) ? intval($req['page']) : 1;
