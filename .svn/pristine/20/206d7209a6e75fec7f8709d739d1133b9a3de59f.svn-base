...
class ExpressHelper extends AbsServer
{
    ...
    public function getAuthCategorys($account_id)
    {
        ...
        $fields ="a.account_id,local_category_id,c.*";
        $categoryService = new CategoryHelp();
        $categorys = AliexpressAccountCategoryPower::where($where)
        ...
        if($detail){
            ...
            if(isset($match[1]) && $match[1]){
                ...
                foreach ($match[1] as $val) {
                    $detailImagePng .= $this->checkPngImages($imgVal,self::$png_image_error['detail_image']);
                }
...
//        }
    ...
    public function deleteProduct(array $arrId)
    {
        ...
        try{
	        foreach ($arrId as $id)
	        {
                AliexpressProduct::destroy($id);
                AliexpressPublishPlan::destroy(['ap_id'=>['in',$id]]);
