...
class GoodsBrandsLink
{
     ...
     * @const int
     */
    public const LOG_FROM_ERP = 1;

    /**
     * 日志来源品连
     * @const int
     */
    public const LOG_FROM_BL = 2;
    ...
    /**
     * 推送品连sku数据列表删除主方法 （只可以删除未推送和 推送失败状态的sku）
     * @param $ids
     * @throws
     */
    public function del($ids)
    {
        $list = GoodsBrandLinkSku::all($ids);
        foreach ($list as $v) {
            if ($v->getData('push_status') == 2) {
                throw new JsonErrorException("sku【：{$v->sku}】，已推送不能删除");
            }
        }
        (new GoodsBrandLinkSku())->where('id', 'in', $ids)->delete();
    }
