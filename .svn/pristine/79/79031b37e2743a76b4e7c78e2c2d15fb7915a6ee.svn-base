...
//        }
    ...
    public function getCustomerHistoerEmails($group_id, $page, $pageSize)
    {
        ...
        $emailList = $listModel->alias('l')
            ...
            ->field('l.*,c.content body')
            ->order('sync_time', 'desc')
            ->page($page, $pageSize)
            ->column('l.*,c.content body');
...
//                    Cache::store('AmazonEmail')->setMaxUid($email, $id);//保存下载的邮件ID
                } catch (Exception $e) {
                    Db::rollback();
                    throw $e;
                }
...
//        $where['status'] = 0;
    ...
    public function emailAccount(){
        ...
        $where['email_allowed_send'] = 1;
        $where['email_allowed_receive'] = 1;
        $where['email_server_id'] = array('neq',0);

        $accounts = Db::table('account')->where($where)->field('account_code')->select();
        if(empty($accounts)){
            return false;
        }
