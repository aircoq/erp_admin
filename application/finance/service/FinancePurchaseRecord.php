...
class FinancePurchaseRecord
{
    ...
    public function __construct()
    {
        ...
        $this->pageSize = $this->request->param('pageSize', 50);
    }
    ...
    public function indexSelectValidate()
    {
        $data = [
            ...
            'supply_chain_specialist_id'   => json_decode($this->request->param('supply_chain_specialist_id'), true),
        ...
        $validate = new FinancePurchaseRecordValidate();
        //check 返回值 boolean
        $bool = $validate->scene('index')->check($data);
    ...
    {
        if (is_numeric($this->request->param($field))) {
    ...
    {
        $timeB = strtotime($this->request->param('date_b'));
        $timeE = strtotime($this->request->param('date_e'));
    ...
    public function listCompleteField($result)
    {
        foreach ($result as &$item) {
            ...
            //金额全部保留两位小数
            $item['sum_shipping_cost']  = number_format($item['sum_shipping_cost'], '2', '.', '');
    ...
    public function detailCompleteField($result)
    {
        ...
        foreach ($result as &$item) {
            ...
            //所有金额四舍五入，保留两位小数
            $item['shipping_cost']          = number_format($item['shipping_cost'], 2, '.', '');
...
p.payment_status,
sum(p.shipping_cost) sum_shipping_cost,
...
FIELD;
    ...
    public function getDetailSql()
    {
        ...
        $sql = $this->baseDetailSql()->where($where)->bind($bind)->buildSql();
