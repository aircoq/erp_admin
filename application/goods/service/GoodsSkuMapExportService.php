...
class GoodsSkuMapExportService
{
    ...
    public function batchSetVirtual($ids, $is_virtual_send, $userInfo)
    {
        ...
        try {
            foreach ($ids as $v) {
                $row = GoodsSkuMap::get($v);
                $oldRow = GoodsSkuMap::get($v)->toArray();
                (new GoodsSkuMapLogService())->mdf($row['channel_sku'], $oldRow, $update)->save($row->id, $userInfo['user_id'], $userInfo['realname']);
            }
