...
class OberloAccountService
{
    ...
    public function getList(array $req = []): array
    {
        ...
        $field = 'am.id,am.sales_company_id,am.base_account_id,am.name,am.code,am.status,am.token_key,am.platform_status,am.is_authorization,am.download_order,am.download_listing,am.sync_delivery,s.site_status,c.seller_id,c.customer_id,a.register_time,a.fulfill_time';
        //有数据就取出
    ...
    public function update($data)
    {
        ...
        try {
            ...
            self::addOberloLog($operator, 1, $new_data, $old_data);
    ...
    public function authorize($data)
    {
        ...
        $log_data = $account->toArray();

        self::addOberloLog($operator, 1, $log_data, $info);
    ...
    public function changeStatus($data)
    {
        ...
        $update_data['update_id'] = $data['update_id'];
        $update_data['update_time'] = input('server.REQUEST_TIME');

        $log_data = $update_data;

        $res = $this->accountModel->allowField(true)->save($update_data, ['id' => $accountInfo->id]);

        $operator = [];
        $operator['operator_id'] = $data['update_id'];
        $operator['operator'] = $data['realname'];
        $operator['account_id'] = $accountInfo->id;
        $res and self::addOberloLog($operator, 1, $log_data, $accountInfo);
        //删除缓存
        Cache::store('OberloAccount')->delAccount();
        return ['message' => '修改成功'];
    }
    ...
    {
        $updateData = [];
        isset($data['platform_status']) && $updateData['platform_status'] = intval($data['platform_status']) ? 1 : 0;
     ...
     * @author lingjiawen
     */
    public static function addOberloLog(array $base_info = [], int $type = 0, array $new_data = [], $old_data = []): void
    {
        $insert_data = [];
        $remark = [];
