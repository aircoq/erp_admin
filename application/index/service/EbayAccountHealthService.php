    ...
    public function saveHealthData(array $data)
    {
        try {
            ...
            $regoinField = [
                ...
                'USA' => 'us',
                'UK,Ireland' => 'uk_ie',
                'Germany, Austria, Switzerland' => 'de_ch_at',
            ];
            $statusTxt = [
                'Standard' => 1,
                'AboveStandard' => 2,
            ...
            foreach ($healths as $k => $health) {
                ...
                if (!empty($health['CurrentSellerLevel'])) {
                    ...
                    //账号表现
                    if (isset($regoinField[$insertData[$k]['region']])) {
                        $performance[$regoinField[$insertData[$k]['region']]] = $statusTxt[$insertData[$k]['current_seller_level']]??0;
                    }
            ...
            if (!$performance) {
                $pfmId = EbayAccountPerformance::where('account_id',$accountId)->order('id desc')->value('id');
                if ($pfmId) {
                    EbayAccountPerformance::update($performance,['id' => $pfmId]);
                }
            }
