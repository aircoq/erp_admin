...
class ManagerServer
{
    ...
    public function getWhereStr($request)
    {
        $params = $request->param();
        $where = "";
        !empty($params['name']) && $where .= ' AND s.a.name like %' . trim($params['name']) . '% ';
        ...
        //&sNum=1&eNum=2
        if (isset($params['sNum']) && isset($params['eNum'])) {
            $where .= ' AND t.`use` between ' . $params['sNum'] . ' AND ' . $params['eNum'];
        } else {
            if ((isset($params['sNum']) && $params['sNum']) > 0) {
                $where .= ' AND t.`use` >= ' . $params['sNum'];
            }

            if ((isset($params['eNum']) && $params['eNum']) > 0) {
                $where .= ' AND t.`use` <= ' . $params['eNum'];
            }
        }
        ...
        switch ($module) {
            ...
            case ChannelNode::module_email: //email
                ...
                if (!empty($info['password'])) {
                    $info['password'] = $encryption->decrypt($info['password']);
                }
                $info['site'] = '';
