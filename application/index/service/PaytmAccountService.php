...
class PaytmAccountService
{
    ...
    public static $log_config = [
        ...
        'client_id_cat' => ['name'=>'应用程序catID','type'=>null],
        'client_secret_cat' => ['name'=>'应用程序密钥','type'=>null],
        'status' => [
            'name'=>'系统状态', 
            'type'=>'list',
            'value'=>[
    ...
    public function getList(array $req)
    {
        ...
        //排序
        $sort_type = !empty($req['sort_type']) && in_array($req['sort_type'], ['account_name', 'code', 'created_at', 'expiry_time']) ? $req['sort_type'] : '';
        ...
        $field = 'am.id,am.account_name,am.code,am.is_invalid status,am.create_time,am.update_time,am.is_authorization,am.sync_delivery,am.download_order,am.download_listing,am.expiry_time,am.paytm_enabled,am.is_authorization_cat,am.mode,s.site_status,c.seller_id,c.customer_id,a.register_time,a.fulfill_time';
        //有数据就取出
        $list = $this->paytmAccountModel
            ->alias('am')
            ->field($field)
            ->join('__ACCOUNT__ a', 'a.id=am.base_account_id', 'LEFT')
            ->join('__CHANNEL_USER_ACCOUNT_MAP__ c', 'c.account_id=am.id AND c.channel_id=a.channel_id', 'LEFT')
        ...
        foreach ($list as &$val) {
            ...
            $val['paytm_enabled'] = $val['expiry_time'] > $time ? 1 : 0;
        }
    ...
    public function add($data, $uid = 0)
    {
        try {
            ...
            if ($id == 0) {
                ...
                $old_data = $this->paytmAccountModel::get($id);
            ...
            $save_id = $this->paytmAccountModel->add($save_data);
    ...
    {
        $field = 'id,account_name,code,base_account_id,is_invalid status,create_time,update_time,is_authorization,sync_delivery,download_order,download_listing,expiry_time,paytm_enabled,is_authorization_cat,mode';
        if ($id == 0) {
    ...
    public function changeStatus($data)
    {
        ...
        try {
            ...
            if (isset($data['status'])) {
                $updata['is_invalid'] = in_array($data['status'], [0,1]) ? $data['status'] : 0;
            }
            $updata['update_time'] = time();

            $new_data = $updata;
            $this->paytmAccountModel->allowField(true)->save($updata, ['id' => $data['id']]) and self::addPaytmLog(
                $operator,
     ...
     * @author lingjiawen
     */
    public static function addPaytmLog(array $base_info = [], int $type = 0, array $new_data = [], $old_data = []): void
    {
        $insert_data = [];
        $remark = [];
        ...
        $insert_data and ChannelAccountLog::addLog([
            'channel_id' => ChannelAccountConst::channel_Paytm,
     ...
     * 获取日志
     */
    public function getPaytmLog(array $req = []): array
    {
        $page = isset($req['page']) ? intval($req['page']) : 1;
