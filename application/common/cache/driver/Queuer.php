...
class Queuer extends Cache
{
    ...
    public function initClearData($hostType, $ipList = null)
    {
    	...
    	$allQueue = \app\common\model\Queue::where('status', 1)->where('host_type', $hostType)->column('queue_class, status, consumer, priority, fail_times, run_times');
    	foreach ($allQueue as $queuer => $attribute){
    		unset($attribute['queue_class']);
    		$queuer = path2class($queuer);
    		$this->restWaitQueue($queuer);
    		//$this->resetQueueHash($queuer, $failData[$queuer]);
    		$this->addQueueHostType($queuer, $hostType, $attribute['priority']);
    		$this->setQueueAttribute($queuer, $attribute);
    	}
    }
    
    public function resetQueueHash($queuer, $failData = [])
    {
    	$timerData = [];
    	...
    	foreach ($hashArr as $params){
    		if(! in_array($params, $queueData) && ! in_array($params, $timerData) && ! in_array($params, $failData)){
