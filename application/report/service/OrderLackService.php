...
class OrderLackService
{
    ...
    {
        $orderOos = new OrderOos();
        $map['oo.lock'] = ['neq', 2];
        $map['s.supply_chain_specialist_id'] = ['gt', 0];
        $supply_chain_specialist_ids = $orderOos->alias('oo')
            ->join('goods g', 'g.id = oo.goods_id', 'left')
            ->join('supplier s', 'g.supplier_id = s.id', 'left')
            ->where($map)
            ->column('s.supply_chain_specialist_id');
        $supply_chain_specialist_ids = array_unique($supply_chain_specialist_ids);

        $list = [];
        foreach ($supply_chain_specialist_ids as $id){
            $user = Cache::store('user')->getOneUser($id, 'id,realname');
            $list[] = ['supply_chain_specialist_id'=>$user['id'],'realname'=>$user['realname']];
        }
        return $list;
    }
    ...
    private function getWhere($params, &$where ,&$join)
    {
        ...
        if ($supplyChainSpecialistId = param($params, 'supply_chain_specialist_id')) {
            if ($supplyChainSpecialistId != 0) {
                $where['s.supply_chain_specialist_id'] = $supplyChainSpecialistId;
            }
        }
    ...
    public function getJoin($params = ''){
        ...
        if (param($params, 'supply_chain_specialist_id')){
            $join['goods'] = ['goods gd', 'gd.id = o.goods_id', 'left'];
            $join['supplier'] = ['supplier s', 'gd.supplier_id = s.id', 'left'];
        }
        return $join;
    }
