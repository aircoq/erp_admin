        ...
        try{
            ...
            if (!$cacheInfo && (!$cacheInfo =
                $listingModel->where(['item_id' => $listingData['listing']['item_id'],'draft'=>0])
                    ->field('id,listing_status,goods_id,local_sku,sku,cost_price,adjusted_cost_price,reserve_price,
                    buy_it_nowprice,sold_quantity')->find())) {
                $id = 0;
                $addFlag = true;
            } else {
                $id = $cacheInfo['id'];
                ...
                if ($id) {//更新
                    ...
                    if($listingData['listing']['variation']){
                        ...
                        if (!$variationList) {//需要新建
                            $tmpVar['goods_id'] = $cacheInfo['goods_id'];
                            $tmpVar['v_sku'] = $cacheInfo['local_sku'];
                            $skuId = GoodsSku::where('sku',$cacheInfo['local_sku'])->value('id');
                            $tmpVar['sku_id'] = empty($skuId) ? 0 : $skuId;
                            $tmpVar['combine_sku'] = $cacheInfo['sku'];
                            $tmpVar['cost_price'] = $cacheInfo['cost_price'];
                            $tmpVar['adjusted_cost_price'] = $cacheInfo['adjusted_cost_price'];
